package registerandlogin.faceserver;
...
import okhttp3.Call;
import registerandlogin.model.FaceRegisterInfo;
...
public class FaceServer {
     ...
     * @return 是否注册成功
     */
    public boolean registerNv21(final Context context, byte[] nv21, int width, int height, FaceInfo faceInfo, String name) {
        synchronized (this) {
            ...
            if (code != ErrorInfo.MOK) {
                ...
                String faceFeatureToString = Base64.encodeToString(faceFeature.getFeatureData(),Base64.NO_WRAP);
                Log.d(TAG, faceFeatureToString);
                OkHttpUtils.post()
                        .url("http://139.196.12.165:8080/AndroidTest/mustAdduser")
                        .addParams("userName",userName)
                        .addParams("faceFeature",faceFeatureToString)
    ...
    public CompareResult getTopOfFaceLib(FaceFeature faceFeature) {
        ...
        for (int i = 0; i < faceRegisterInfoList.size(); i++) {
            ...
            int compareCode = faceEngine.compareFaceFeature(faceFeature, tempFaceFeature, faceSimilar);
            Log.d(TAG,"错误码为"+compareCode);
